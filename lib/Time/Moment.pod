...
total of nine digits. When the fractional precision is greater than nine 
...
[z]. The literal string GMT or UTC may be used as a UTC designator and may
have an offset. Usage of these string representations is strongly discouraged 
as they do not conform to the ISO 8601 standard.
...
Returns the Julian Date. Julian Date is a count of days elapsed since 
Greenwich Mean Time noon on January 1, 4713 BCE (proleptic Julian calendar). 
The fractional part corresponds to the fraction of the day after noon.

=head2 mjd

    $mjd = $tm->mjd;

Returns the Modified Julian Date. Modified Julian Date is a count of days 
elapsed since Greenwich Mean Time midnight on Novemeber 17, 1858. The 
...
local date or time value or altering the components of the local date 
or time it may be necessary to use a time zone to convert to the correct 
representation. 
