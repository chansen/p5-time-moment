...
the nano of second [0, 999_999_999]. The optional parameter I<offset> 
specifies the offset from UTC in minutes [-1080, 1080].
...
attempt is made to coerce the object to an instance of C<Time::Moment>.

C<Time::Moment> provides coercion handlers for the following object types:
...
have a time zone or an offset from UTC, coercing from the 'floating' time zone
is not supported.
...
Returns the micro of second [0, 999_999].

=head2 nanosecond

    $nanosecond = $tm->nanosecond;

Returns the nano of second [0, 999_999_999].
...
Returns a copy of this time with the given I<offset> altered. The resulting 
time is at the same instant.
...
Replaced by the fractional second including preceding decimal point or by 
an empty string if no fractional second exists.

    .fff        (millisecond)
    .ffffff     (microsecond)
    .fffffffff  (nanosecond)

The shortest representation is used.

I<Please note that this this conversion specification is an extension to the 
L<"IEEE Std 1003.1, 2013 Edition"|http://pubs.opengroup.org/onlinepubs/9699919799/functions/strftime.html>>.
...
specification permits use of a optional width flag where the default width 
of C<0> will use the shortest representation.
...
C<%N4> is replaced by exactly four fractional digits, zero padded or truncated
if needed.
