...
in the ISO 8601 calendar with nanosecond resolution.
...
decimal fraction. The constructor raises an exception if the given string 
cannot be parsed.
...
Combined with a zone designator in one of the following formats:

    Z
    ±hh
    ±hh:mm

The shortest representation will be used where the omitted components are 
implied to be zero.
...
Equivalent to C<%Y-%m-%dT%H:%M:%S%f%Z>.
...
an empty string if the nano of second is C<0>.
...
Replaced by the fractional second as a decimal number. This specifier
permits use of a optional width where the default width of C<0> will use the 
...
C<%N4> is replaced by exactly four fractional digits.
