...
C<Time::Moment> is an immutable object that represents a specific date and 
time of day, along with an offset from UTC, within the ISO 8601 calendar 
system. Time is measured in nanoseconds since C<0001-01-01T00Z>. Leap seconds 
are not accounted for in C<Time::Moment>; each day is assumed to have exactly 
C<86,400,000,000,000> nanoseconds.

Time::Moment supports all epoch integers from C<-62,135,596,800> to 
C<253,402,300,799>, allowing for nanosecond precision for any instant within 
the range C<0001-01-01T00:00:00Z> to C<9999-12-31T23:59:59Z>.
...
epoch of 1970-01-01T00Z. Fractional seconds are supported when only seconds 
are provided, supplying a nanosecond value takes precedence. The default 
precision is of the fractional seconds is 6 (microseconds).
...
Returns the week of the week-based year [1, 53].
...
Returns a copy of this instance with the given I<week> of the week-based year [1, 53] 
...
Returns the difference between the B<local date> of this moment and that of 
I<other> in terms of whole B<calendar years>. The result will be negative 
if the local date of the I<other> moment is before this.

The calculation returns an integer and represents the number of complete
calendar years separating the two dates. Partial years do not count toward 
the result. For example, the difference between C<2010-06-15> and 
C<2012-06-14> is one year, because it is one day short of a full two-year 
interval.

Conceptually, the method counts how many complete year boundaries can be
crossed when moving from this moment toward I<other> (or vice versa),
without passing the target. Only the local date participates in the 
calculation.

As with months, year calculations may involve asymmetric edge cases due to
leap years. For instance, advancing from February 29 normalises to
February 28 in non-leap years.

See L</TIME AND CALENDAR ARITHMETIC>.
...
Returns the difference between the B<local date> of this moment and that of 
I<other> in terms of whole B<calendar months>. The result will be negative 
if the local date of the I<other> moment is before this.

The calculation returns an integer and reflects the number of complete calendar
months separating the two dates. Partial months do not count toward the result. 
For example, the difference between C<2025-10-11> and C<2025-12-10> is one 
month, because it falls one day short of a full two-month interval.

Conceptually, the method counts how many complete month boundaries can be
crossed when moving from this moment toward I<other> (or vice versa) without
passing the target, using calendar-month arithmetic. Only the local date 
participates in the calculation.

Because calendar months vary in length, and dates such as the 29th, 30th, and
31st are not present in every month, some edge cases are asymmetric. For
example, advancing from the 31st normalises the day to the last valid date in
...
Returns the difference between the B<local date> of this moment and that of 
I<other> in terms of whole B<calendar weeks>. The result will be negative 
if the local date of the I<other> moment is before this.

The calculation returns an integer and represents the number of complete
seven-day calendar intervals separating the two dates. Partial weeks do not 
count toward the result.

Conceptually, the method counts how many complete seven-day boundaries can be
crossed when moving from this moment toward I<other> (or vice versa) without
passing the target. Only the local date participates in the calculation.

This calculation is not affected by timezone offsets, except insofar as they 
determine which local date each moment falls on.
...
Returns the difference between the B<local date> of this moment and that of 
I<other> in terms of whole B<calendar days>. The result will be negative 
if the local date of the I<other> moment is before this.

The calculation returns an integer and represents the number of complete
calendar days separating the two dates. Partial days do not count toward 
the result.

Conceptually, the method counts how many calendar-day boundaries (midnights)
can be crossed when moving from this moment toward I<other> (or vice versa)
without passing the target. Only the local date participates in the 
calculation.
...
Returns the difference between the B<instant> of this moment and the I<other> 
in terms of whole hours. The result will be negative if the instant of the 
...
Returns the difference between the B<instant> of this moment and the I<other> 
in terms of whole minutes. The result will be negative if the instant of the 
I<other> moment is before this.
...
Returns the difference between the B<instant> of this moment and the I<other> in 
terms of whole seconds. The result will be negative if the instant of the I<other> 
moment is before this.
...
Returns the difference between the B<instant> of this moment and the I<other> in 
terms of whole milliseconds. The result will be negative if the instant of the 
I<other> moment is before this.
...
Returns the difference between the B<instant> of this moment and the I<other> in 
terms of whole microseconds. The result will be negative if the instant of the 
I<other> moment is before this.
...
Returns the difference between the B<instant> of this moment and the I<other> in 
terms of whole nanoseconds. The result will be negative if the instant of the 
I<other> moment is before this.
...
Time::Moment primarily supports four distinct calendars through C<< ->from_string >> as well as accessor and adjustor (C<< ->with >>) methods.

=head2 Calendar Date (ISO 8601)

The calendar date format represents a traditional date system based on the year, month, and day. It is structured as C<YYYY-MM-DD> where C<YYYY> is the year, C<MM> is the two-digit month number (ranging from 01 for January to 12 for December), and C<DD> is the two-digit day of the month (ranging from 01 to 31, depending on the month).

Examples in both basic and extended formats:

=over

=item * December 17, 2024

=over

=item * Extended format: C<2024-12-17>

=item * Basic format: C<20241217>
...
=item * Extended format: C<2024-07-01>  

=item * Basic format: C<20240701>

=back

=back

To construct a Time::Moment instance for July 1, 2024:

    $tm = Time::Moment->from_string('20240701T00Z');
    $tm = Time::Moment->from_string('2024-07-01T00Z');
    $tm = Time::Moment->new(year => 2024, month => 7, day => 1); 

See L</Date> for example L</strftime> conversion specifications.

=head2 Ordinal Date (ISO 8601)

The ordinal date format represents a date using the year and the day of the year. It is structured as C<YYYY-DDD>, where C<YYYY> represents the year, and C<DDD> is the day of the year, a three-digit number ranging from C<001> to C<365> (or C<366> for leap years).

Examples in both basic and extended formats:

=over

=item * First day of 2024

=over

=item * Extended format: C<2024-001> 

=item * Basic format: C<2024001>

=back

=item * 366th day of 2024 (last day of the year, leap year)  

=over

=item * Extended format: C<2024-366>  

=item * Basic format: C<2024366>

=back

=back

To construct a Time::Moment instance for the 366th day of 2024:

    $tm = Time::Moment->from_string('2024366T00Z');
    $tm = Time::Moment->from_string('2024-366T00Z');
    $tm = Time::Moment->new(year => 2024)
                      ->with_day_of_year(366);

See L</Date> for example L</strftime> conversion specifications.

=head2 Week Date (ISO 8601)

The week date format is a standardized way to represent dates based on the week of the year. It is structured as C<YYYY-Www-D>, where C<YYYY> represents the week-based year, which may differ from the calendar year in certain cases. C<Www> is the two-digit week number, ranging from C<01> to C<53>, indicating the specific week of the year. The final component, C<D>, represents the day of the week, with values from C<1> (Monday) to C<7> (Sunday).

The system standardizes the structure of years by defining the first week (C<W01>) as the week that includes the first Thursday of the calendar year. Consequently, most years consist of 52 weeks, but some may have 53 weeks depending on how the weeks align with the calendar.

The week year may differ from the calendar year at the beginning or end of the year, for example:

=over

=item * If January 1 falls on a Friday, Saturday, or Sunday, those days will belong to the last week of the previous week year.

=item * Similarly, if December 31 falls on a Monday, Tuesday, or Wednesday, those days will belong to the first week of the next week year.

=back

This system ensures consistency for weeks but can cause overlaps or shifts between calendar and week years.

Examples in both basic and extended formats:

=over

=item * Monday of the first week of 2024  

=over

=item * Extended format: C<2024-W01-1>

=item * Basic format: C<2024W011>

=back

=item * Sunday of the 52nd week of 2023

=over

=item * Extended format: C<2023-W52-7>

=item * Basic format: C<2023W527>

=back

=back

Note: The standard defines week 01 as the week containing the first Thursday of the calendar year, which is equivalent to the week that includes January 4, due to the interdependent properties of the week.

To construct a Time::Moment instance for Sunday of the 52nd week of 2023:

    $tm = Time::Moment->from_string('2023W527T00Z');
    $tm = Time::Moment->from_string('2023-W52-7T00Z');
    $tm = Time::Moment->new(year => 2023, day => 4)
                      ->with_week(52)
                      ->with_day_of_week(7);

See L</Date> for example L</strftime> conversion specifications.

=head2 Quarter Date (Non-Standard)

The quarter date format represents a date based on the year, the quarter of the year, and day within the quarter. It is structured as C<YYYY-Qq-dd>, where C<YYYY> is the year, C<Qq> indicates the quarter (C<Q1>, C<Q2>, C<Q3>, or C<Q4>), and C<dd> is the day of the quarter, ranging from C<01> to C<92> depending on the quarter. For example, C<Q1> refers to the first quarter, which includes January, February, and March; C<Q2> refers to the second quarter (April, May, and June), and so on.

Examples in both basic and extended formats:

=over

=item * First day of the first quarter of 2024 

=over

=item * Extended format: C<2024-Q1-01>  

=item *  Basic format: C<2024Q101>

=back

=item * 45th day of the second quarter of 2023 

=over

=item * Extended format: C<2023-Q2-45> 

=item * Basic format: C<2023Q245>

=back

=back

To construct a Time::Moment instance for 45th day of the second quarter of 2023:

    $tm = Time::Moment->from_string('2023Q245T00Z');
    $tm = Time::Moment->from_string('2023-Q2-45T00Z');
    $tm = Time::Moment->new(year => 2023)
                      ->with_quarter(2)
                      ->with_day_of_quarter(45);

=head1 TIME AND CALENDAR ARITHMETIC

Time and Calendar Arithmetic refers to operations involving the addition, subtraction, or comparison of temporal values. These operations can involve either B<precise time units> (such as seconds, minutes, and hours) or B<calendar units> (like days, months, and years). These two types of arithmetic are fundamentally different due to their distinct characteristics.

Mixing B<calendar units> and B<time units> can lead to confusion and inaccuracies because they adhere to different principles:

=head2 1. Calendar Units Are Variable

=over

=item *

B<Days, months, and years> do not have consistent lengths:

=over

=item *

A calendar day represents a transition between local midnights. While it
often spans 24 hours, daylight saving time changes and other adjustments
may cause it to be shorter or longer.

=item *

A month can vary between 28, 29, 30, or 31 days.

=item *

A year can be 365 or 366 days (leap years).
...
=head2 2. Conceptual Differences Between Calendar and Time Units

=over

=item *

B<Calendar units> are positional and relate to transitions within a calendar system. For example, moving from "December 31 to January 1" indicates crossing into a new day, even if only 1 second has passed.

=item *

B<Time units> measure exact durations, such as the total number of seconds, minutes, or hours between two timestamps.

=back

B<Example:>  
The difference between C<2012-12-31T23:59:59Z> and C<2013-01-01T00:59:59Z>:

=over

=item *

In B<calendar terms>, this spans 2 calendar days, as moving even 1 second into January 1 is considered a new day.

=item *

In B<time units>, the difference is precisely 1 hour or 60 minutes.

=back

=head2 3. Risks of Mixing Units

=over

=item *

Mixing calendar and time units can lead to misleading interpretations:
...
=head2 When to Use Each Type of Arithmetic

=over

=item *

Use B<calendar units> when dealing with schedules, recurring events, or positional changes within a date system (e.g., adding months to calculate an anniversary).

=item *

Use B<time units> when working with precise durations, such as timers, interval measurements, or elapsed time calculations.

=back

=head2 Boundary-Crossing Model for Calendar Arithmetic

Calendar-based delta methods operate by counting how many complete calendar
unit boundaries can be crossed when moving from one local date toward the
other, without passing the target date.
...
Identify the earlier and later local dates.

=item 2.

Starting from the earlier date, repeatedly advance by one calendar unit (year, month, week, or day).

=item 3.

Count how many such advances can be made without exceeding the later date.

=back

Partial units are ignored. Only complete boundary crossings are counted.

=head2 Choosing the Correct Delta Method

The following table provides a quick reference for selecting the
appropriate delta method. It distinguishes between calendar-based and
time-based deltas.
...
Calendar-based deltas operate on the local date and count whole calendar
units. This calculation is not affected by timezone offsets, except
insofar as they determine which local date each moment falls on.

=item *

Time-based deltas operate on the exact instant in time and measure elapsed
duration using fixed-length time units. These calculations are based on 
absolute instants and are therefore independent of local calendar effects, 
though the instants themselves may originate from timezone-aware moments.

=back

    +----------------------+-------------+-----------+
    | Method               | Based on    | Unit type |
    +----------------------+-------------+-----------+
    | delta_years          | Local date  | Calendar  |
    | delta_months         | Local date  | Calendar  |
    | delta_weeks          | Local date  | Calendar  |
    | delta_days           | Local date  | Calendar  |
    | delta_hours          | Instant     | Time      |
    | delta_minutes        | Instant     | Time      |
    | delta_seconds        | Instant     | Time      |
    | delta_milliseconds   | Instant     | Time      |
    | delta_microseconds   | Instant     | Time      |
    | delta_nanoseconds    | Instant     | Time      |
    +----------------------+-------------+-----------+

Consider the following two moments:

  C<2012-12-31T23:59:59Z> and C<2013-01-01T00:59:59Z>

Only one hour of time has elapsed between these instants, yet a calendar
day boundary has been crossed.

  delta_days   = 1
  delta_hours  = 1

Use C<delta_days> when the question is:

  "How many calendar days apart are these dates?"

Use C<delta_hours> when the question is:

  "How much time actually elapsed between these moments?"

Although only one hour has elapsed, the calendar-based and time-based
results differ because calendar arithmetic counts date boundaries, not
durations.

=head1 TIME ZONES

A C<Time::Moment> instance represents a precise point in time but does 
