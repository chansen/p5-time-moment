...
Combinations of calendar date and local time and the difference from UTC
...
Combinations of ordinal date and local time and the difference from UTC
...
Combinations of week date and local time and the difference from UTC
...
=head4 RFC 1123 - Requirements for Internet Hosts -- Application and Support
...
=head4 RFC 2616 - HTTP/1.1

L<RFC 2616|http://tools.ietf.org/html/rfc2616#section-3.3.1>.
...
An HTTP date value represents time as an instance of UTC:
    ...
    $string = Time::Moment->now_utc->strftime("%a, %d %b %Y %H:%M:%S GMT");
    $string = $tm->with_offset(0)->strftime("%a, %d %b %Y %H:%M:%S GMT");

=head4 RFC 5322 - Internet Message Format

L<RFC 5322|http://tools.ietf.org/html/rfc5322#section-3.3>.
