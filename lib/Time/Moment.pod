...
=head1 SYNOPSIS
    ...
    $tm2          = $tm1->at_midnight;              # T00:00:00.000000000
    $tm2          = $tm1->at_noon;                  # T12:00:00.000000000
...
Time is measured in nanoseconds since C<1970-01-01T00Z>. In C<Time:Moment> 
...
all epoch integers from C<-62,135,596,800> to C<2,534,02,300,799>; this 
...
The time zone offset from UTC in minutes [-1080, 1080] (±18:00).
...
epoch of 1970-01-01T00Z. The optional parameter I<nanosecond> specifies the 
nanosecond of the second [0, 999_999_999]. Fractional seconds is supported 
if the constructor is invoked with I<seconds> only:
...
followed by a time zone designator. The time of day may be reduced to hour 
...
=head2 from_jd
    ...
    $tm = Time::Moment->from_jd($jd);
    $tm = Time::Moment->from_jd($jd [, precision => 3] [, epoch => -1721425.5]);
...
The optional parameter I<precision> [0, 9] specifies the number of decimal
digits after the decimal point. The default precision is 3.

=item epoch

    $tm = Time::Moment->from_jd($jd, epoch => -1721425.5);

The optional parameter I<epoch> specifies the epoch date relative to 
0001-01-01T00Z. The epoch date for the Julian Date is C<-1721425.5>.
...
=head2 from_mjd
    ...
    $tm = Time::Moment->from_mjd($mjd);
    $tm = Time::Moment->from_mjd($mjd [, precision => 3] [, epoch => 678575]);
...
=item epoch
    ...
    $tm = Time::Moment->from_mjd($mjd, epoch => 678575);

The optional parameter I<epoch> specifies the epoch date relative to 
0001-01-01T00Z. The epoch date for the Modified Julian Date is C<678575>.
...
Returns the time zone offset from UTC in minutes [-1080, 1080] (±18:00).
...
altered. The nanosecond of the second is unaltered.
...
in minutes [-1080, 1080] (±18:00) altered. The resulting time is at the same 
instant.
...
Returns a copy of this instance with the given time zone I<offset> from UTC 
in minutes [-1080, 1080] (±18:00) altered. The resulting time has the same 
local time.
...
Returns a copy of this instance with the time zone offset set to zero.
...
T00:00:00.000000000. This method is equivalent to:
...
T12:00:00.000000000. This method is equivalent to:
...
The total length of the string representation is between 20 and 35 
...
Copyright 2013-2015 by Christian Hansen.
