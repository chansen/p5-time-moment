...
Returns the difference between the B<local date> of this moment and the 
...
The calculation returns a whole number and represents the number of complete
calendar years separating the two dates. Partial years do not contribute to
the result. For example, the difference between C<2010-06-15> and
C<2012-06-14> is only one year, because it is one day short of a full
two-year interval.

This method conceptually counts how many complete year boundaries can be
crossed when moving from C<$tm> toward C<$other> (or the reverse), without
passing the target. Only the local date participates in the calculation.

As with months, year calculations can involve asymmetric edge cases due to
leap years. For instance, stepping forward from February 29 will normalise
to February 28 in non-leap years.

Use C<delta_years>, C<delta_months>, C<delta_weeks> and C<delta_days> 
for calendar-based  calculations, such as recurrence rules, schedules, 
and billing cycles. For elapsed-time measurements, use C<delta_hours>, 
C<delta_minutes>, or C<delta_seconds> instead.
...
The calculation returns a whole number and represents the number of complete
calendar months separating the two dates. Partial months do not count toward
the result. For example, the difference between C<2025-10-10> and
C<2025-12-11> is only one month, because it is one day short of an exact
two-month interval.

This method conceptually counts how many complete month boundaries can be
crossed when moving from C<$tm> toward C<$other> (or the reverse), using
calendar-month arithmetic without passing the target. Only the local date
participates in the calculation.

Calendar months have different lengths, and dates such as the 29th, 30th,
and 31st do not exist in every month. As a result, certain edge cases are
asymmetric. For example, stepping forward from the 31st normalises the day
...
Returns the difference between the B<local date> of this moment and the I<other> 
in terms of whole B<calendar weeks>. The result will be negative if the local date of 
...
Returns the difference between the B<local date> of this moment and the I<other> 
in terms of whole B<calendar days>. The result will be negative if the local date of 
the I<other> moment is before this.
...
A day is generally 24 hours, but events like daylight saving time changes or leap seconds can alter this.
