...
This method modifies the offset of an object while maintaining the exact 
moment in time (the "instant"). It ensures that the resulting object reflects 
the same point in time but with a different offset.

Use this method to convert between time zones without altering the absolute time,
or when you need to represent a specific moment in a different time zone for 
purposes such as reporting or logging.
...
This method modifies the offset of an object while preserving the local time. 
It is commonly used in scenarios where the local time stays the same, but the 
offset changes, such as during daylight saving time transitions.

Use this method to apply an offset that represents the same local time in a 
different time zone or to adjust offsets during ambiguous local times, like 
those occurring in daylight saving changes.
...
in terms of complete B<calendar years>. The result will be negative if the local date of 
...
Returns the difference between the B<local date> of this moment and the I<other> 
in terms of complete B<calendar months>. The result will be negative if the local date of 
the I<other> moment is before this.
...
Returns the difference between the B<local date> of this moment and the I<other> 
in terms of complete B<calendar weeks>. The result will be negative if the local date of 
the I<other> moment is before this.
...
Returns the difference between the B<local date> of this moment and the I<other> 
in terms of complete B<calendar days>. The result will be negative if the local date of 
the I<other> moment is before this.
